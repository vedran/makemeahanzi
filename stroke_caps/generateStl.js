...
function generateOpenScad(char, options = {}) {
  ...
  const romanAreaHeight = 15; // Space for romanization text at bottom (mm)
  const romanTextSize = 6; // Text size in mm
  const romanTextY = 8; // Distance from bottom edge to text center (mm)
...
module character_tile() {
    difference() {
        ...
        // Recessed character strokes with arrows/numbers cut out
        // The arrows remain at plate surface level (not recessed) - same height as filled strokes
        // This creates no tactile difference but visual difference in color
...
${romanization ? `
// Romanization text (positioned at bottom of tile)
