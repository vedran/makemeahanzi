...
function generateOpenScad(char, options = {}) {
  ...
  const arrowLineWidth = 8; // Width of arrow line in SVG units
...
`;
  ...
  for (let i = 0; i < strokes.length; i++) {
    ...
    // Add stroke number (45px in SVG = ~4.5mm in model)
    const textSize = 45 * SCALE;
    ...
    arrowPoints.push([baseX, baseY]);

    // Create arrow line as hull of circles
    if (arrowPoints.length >= 2) {
      scadCode += `        hull() {\n`;
      for (const pt of arrowPoints) {
        const [px, py] = svgToModelCoords(pt[0], pt[1]);
        scadCode += `            translate([${px.toFixed(3)}, ${py.toFixed(3)}]) circle(r=${(arrowLineWidth/2 * SCALE).toFixed(3)});\n`;
      }
      scadCode += `        }\n`;
    }
