...
function generateOpenScad(char, options = {}) {
  ...
  const plateHeight = romanization ? PLATE_SIZE_MM * 1.2 : PLATE_SIZE_MM; // 20% taller for text
  const romanTextY = 8; // Position from bottom edge in mm
  const romanTextSize = 12; // Text size in mm
...
module character_tile() {
    difference() {
        ...
        translate([0, ${romanization ? (plateHeight - PLATE_SIZE_MM).toFixed(2) : 0}, recess_start])
...
module romanization_text() {
    translate([plate_width / 2, ${romanTextY}])
        rotate([0, 0, 180])
            text("${romanization}", size=${romanTextSize}, halign="center", valign="bottom", font="Arial:style=Bold");
}
` : ''}
