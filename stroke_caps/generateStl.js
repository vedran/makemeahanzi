...
function generateOpenScad(char, options = {}) {
  ...
  const arrowLineWidth = 1.2; // Width of arrow line in SVG units (~12.5% of typical stroke width)
...
`;
  ...
  for (let i = 0; i < strokes.length; i++) {
    ...
    for (let j = 0; j < arrowPoints.length - 1; j++) {
      const [px1, py1] = svgToModelCoords(arrowPoints[j][0], arrowPoints[j][1]);
      const [px2, py2] = svgToModelCoords(arrowPoints[j + 1][0], arrowPoints[j + 1][1]);
